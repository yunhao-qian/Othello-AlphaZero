...
cdef size_t _get_best_child(
    ...
    cdef float sqrt_sum_visit_count = sqrtf(<float>sum_visit_count)

    # Compute Dirichlet noises which encourage additional exploration.

    cdef vector[float] noises
    noises.reserve(children.size())

    cdef float noise
    cdef gamma_distribution[float] gamma_dist = gamma_distribution[float](
        context.dirichlet_alpha, 1.0
    )
    cdef float noise_sum = 0.0
...
cdef vector[size_t] _forward_pass(MCTSContext &context, mt19937 &rng) noexcept nogil:
    ...
    for i in range(1, search_path.size()):
        ...
        child.mean_action_value = (
            child.total_action_value / <float>child.visit_count
        )

    return search_path


cpdef float get_terminal_position_action_value(Position &position) noexcept nogil:
...
cpdef object create_net_input(Position &position):
    ...
    cdef float player = <float>(position.player == 2)

    with nogil:
        for row in range(8):
            for col in range(8):
                features_view[0, row, col] = <float>(
                    (position.p1_discs & square_mask) != 0
                )
                features_view[1, row, col] = <float>(
                    (position.p2_discs & square_mask) != 0
                )
                features_view[2, row, col] = player
                square_mask >>= 1

    array = np.asarray(features_view)
...
cdef void _simulate_once(
    ...
    cdef float action_value
    cdef vector[int] legal_actions
    cdef float *policy_data
    cdef float *value_data
    cdef MCTSNode *child
    ...
    with nogil:
        ...
        # Saving leaf position on stack somehow makes the code faster.
        leaf_position = leaf.position

        # Expand and evaluate the leaf node.
        if is_terminal_position(leaf_position):
            # If the game is over, we do not need neural network evaluation.
            action_value = get_terminal_position_action_value(leaf_position)
        else:
            ...
            with gil:
                ...
                policy_data = <float *><unsigned long long>policy.data_ptr()
                value_data = <float *><unsigned long long>value.data_ptr()
            ...
            if leaf.children.empty():
                ...
                for action in legal_actions:
                    ...
                    context.search_tree.emplace_back()

                    child = &context.search_tree.back()
                    child.position = apply_action(leaf_position, action)
        ...
        for i in range(1, search_path.size()):
            ...
            # +1 to cancel out -1 in the virtual loss.
            # The action-value is with respect to the parent node, so we need to flip
            # the sign.
...
cdef size_t _collect_subtree(
    ...
    for old_child_index in old_node.children:
        new_node.children.push_back(
            _collect_subtree(old_tree, new_tree, old_child_index)
        )
...
cdef vector[MCTSNode] _prune_search_tree(
    ...
    # Reserve the maximum possible size so that pointers are never invalidated.
...
cdef class MCTSPlayer:
    ...
    def __init__(
        ...
        self.reset_position(get_initial_position())
